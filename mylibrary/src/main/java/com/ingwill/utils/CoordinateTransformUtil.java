...
import com.ingwill.entity.ExifInfo;
...
public class CoordinateTransformUtil {
    ...
    public static boolean out_of_china(double lng, double lat) {
     ...
     public static Location exif2Loc(String flNm) {
         ...
         try {
             ...
             String sDate =ef.getAttribute(ExifInterface.TAG_DATETIME);
             Date date = DateUtils.string2Date(sDate);
    ...
    public static ExifInfo getExifInfo(String flNm) {
        String sLat = "", sLatR = "", sLon = "", sLonR = "";
        Date date=null;
        try {
            ExifInterface ef = new ExifInterface(flNm);
            ...
            if(!StringUtils.isNullOrEmpty(sDate)) {
                date = DateUtils.string2Date(sDate,"yyyy:MM:dd HH:mm:ss");
            }
        } catch (IOException e) {return null;}
        double lat = dms2Dbl(sLat);
        if (lat > 180.0) return null;
    ...
    public  static Address getAddress(Context context,ExifInfo location) {
...
//                if(!StringTools.isNullOrEmpty(ad.getAddressLine(1))) {
//                    sb.append(ad.getAddressLine(1));
//                }
//                if(!StringTools.isNullOrEmpty(ad.getAddressLine(2))) {
...
//                }
    ...
    public  static Address getAddress(Context context,String name) {
        ...
        //StringBuilder sb = new StringBuilder();
        try {
            List<Address> add = gc.getFromLocationName(name, 1);
            if (add.size() > 0) {
                return add.get(0);
