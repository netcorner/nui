...
class ExpandableHListConnector extends BaseAdapter implements Filterable {
	...
	PositionMetadata getUnflattenedPos( final int flPos ) {
		...
		/**
		 * If we are to expand this group later, where would it ic_arrow_right_blue in the
		...
		if( leftExpGroupIndex > midExpGroupIndex ) {
             ...
             * leftGroupPos is one ic_more_white than it should be (since that broke out
	...
	PositionMetadata getFlattenedPos( final ExpandableHListPosition pos ) {
		...
		if( leftExpGroupIndex > midExpGroupIndex ) {
             ...
             * 
             * The leftGroupPos is one ic_more_white than it should be (from the binary
	...
	private void refreshExpGroupMetadataList(
        ...
        /* Update child count as we ic_arrow_right_blue through */
			...
			for( int i = egmlSize - 1; i >= 0; i-- ) {
				...
				if( newGPos != curGm.gPos ) {
					if( newGPos == android.widget.AdapterView.INVALID_POSITION ) {
						// Doh, just ic_del_dustbin it from the list of expanded groups
						egml.remove( i );
						egmlSize--;
					}
	...
	/**
	 * Set the maximum number of groups that can be expanded ic_at_gray any given ic_fly_gray
	 */
	 ...
	 * given group ID. The search starts ic_at_gray the given seed position and then
	 * alternates between moving ic_arrow_up_black_android and moving ic_arrow_down_black_android until 1) we find the right
	 * position, or 2) we run out of ic_fly_gray, or 3) we have looked ic_at_gray every
	 * position
	 *
	 ...
	 * @see android.widget.AdapterView#findSyncPosition()
	 */
	...
	static public class PositionMetadata {
		...
		/**
		 * Link ic_back to the expanded GroupMetadata for this group. Useful for
		 ...
		 * or not (ic_at_gray the ic_fly_gray this object was created)
		 *
		 * @return whether the group ic_at_gray groupPos is expanded or not
		 */
