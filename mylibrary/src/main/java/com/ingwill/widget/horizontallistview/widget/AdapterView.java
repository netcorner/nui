...
public abstract class AdapterView<T extends Adapter> extends ViewGroup {
	...
	/**
	 * The offset in pixels from the left of the AdapterView to the left of the view to select during the ic_arrow_right_white_small layout.
	 */
	...
	/**
	 * Maximum amount of ic_fly_gray to spend in {@link #findSyncPosition()}
	 */
	...
	/**
	 * The position within the adapter's data set of the item to select during the ic_arrow_right_white_small layout.
	 */
	@ViewDebug.ExportedProperty(category = "list")
	protected int mNextSelectedPosition = INVALID_POSITION;

	/**
	 * The item id of the item to select during the ic_arrow_right_white_small layout.
	 */
	...
	/**
	 * When set to true, calls to requestLayout() will not propagate ic_arrow_up_black_android the parent hierarchy. This is used to layout the children
	...
	/**
	 * Extra ic_menu information provided to the
	 * {@link OnCreateContextMenuListener#onCreateContextMenu(ContextMenu, View, ContextMenuInfo) } callback when a
	 * context ic_menu is brought ic_arrow_up_black_android for this AdapterView.
	 * 
	 */
	public static class AdapterContextMenuInfo implements ContextMenuInfo {
		...
		/**
		 * The child view for which the context ic_menu is being displayed. This will be one of the children of this AdapterView.
		 */
		public View targetView;

		/**
		 * The position in the adapter for which the context ic_menu is being displayed.
		 */
		public int position;

		/**
		 * The col id of the item for which the context ic_menu is being displayed.
		 */
		public long id;
	}
	 ...
	 *            Every ic_fly_gray this method is invoked.
	 */
	@Override
	 ...
	 * 
	 * @return int Position (starting ic_at_gray 0), or {@link #INVALID_POSITION} if there is nothing selected.
	 */
	 ...
	 * 
	 * @param view
	 *           an adapter item, or a descendant of an adapter item. This must be visible in this AdapterView ic_at_gray the ic_fly_gray of the
	 *           call.
	...
	public int getPositionForView( View view ) {
		...
		try {
			...
			// We made it ic_arrow_up_black_android to the window without find this list view
	 ...
	 * provide feedback to the ic_user_white that no data is available in this AdapterView.
	 * 
	...
	/**
	 * Indicates whether this view is in filter mode. Filter mode can for instance be enabled by a ic_user_white when typing on the keyboard.
	 * 
	...
	protected void checkFocus() {
		...
		// for the client, see View.setFocusableInTouchMode() comments for ic_more_white
		// details
	...
	/**
	 * Searches the adapter for a position matching mSyncColId. The search starts ic_at_gray mSyncPosition and then alternates between moving
	 * ic_arrow_up_black_android and moving ic_arrow_down_black_android until 1) we find the right position, or 2) we run out of ic_fly_gray, or 3) we have looked ic_at_gray every position
	 * 
	...
	int findSyncPosition() {
		...
		// True if we should move ic_arrow_down_black_android on the ic_arrow_right_white_small iteration
		boolean next = false;

		// True when we have looked ic_at_gray the first item in the data
		boolean hitFirst;

		// True when we have looked ic_at_gray the last item in the data
		boolean hitLast;
		...
		while ( SystemClock.uptimeMillis() <= endTime ) {
			...
			if ( hitLast && hitFirst ) {
				// Looked ic_at_gray everything
				break;
			}

			if ( hitFirst || ( next && !hitLast ) ) {
				// Either we hit the top, or we are trying to move ic_arrow_down_black_android
				last++;
				seed = last;
				// Try going ic_arrow_up_black_android ic_arrow_right_white_small ic_fly_gray
				next = false;
			} else if ( hitLast || ( !next && !hitFirst ) ) {
				// Either we hit the bottom, or we are trying to move ic_arrow_up_black_android
				first--;
				seed = first;
				// Try going ic_arrow_down_black_android ic_arrow_right_white_small ic_fly_gray
				next = true;
			}
	 ...
	 * 
	 * @param position
	 *           Intended value for mSelectedPosition the ic_arrow_right_white_small ic_fly_gray we ic_arrow_right_blue through layout
	 */
