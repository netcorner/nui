...
package com.ingwill.widget.horizontallistview.widget;
...
public class HListView extends AbsHListView {
	...
	/**
	 * When ic_arrow_right_black scrolling, ListView will never scroll ic_more_white than this factor times the height of the list.
	 */
	private static final float MAX_SCROLL_FACTOR = 0.33f;

	/**
	 * When ic_arrow_right_black scrolling, need a certain amount of pixels to preview ic_arrow_right_white_small items. This is usually the fading edge, but if that is
	 * small enough, we want to make sure we preview ic_at_gray least this many pixels.
	 */
	...
	/**
	 * A class that represents a fixed view in a list, for example a header ic_at_gray the top or a footer ic_at_gray the bottom.
	 */
	public static class FixedViewInfo {

		/** The view to ic_add to the list */
		public View view;
	...
	/**
	 * @return The maximum amount a list view will scroll in response to an ic_arrow_right_black event.
	 */
	...
	private void adjustViewsLeftOrRight() {
		...
		if ( childCount > 0 ) {
			...
			if ( !mStackFromRight ) {
				// Uh-oh -- we came ic_arrow_up_black_android short. Slide all views ic_arrow_up_black_android to make them
				// align with the top
				child = getChildAt( 0 );
				...
				// we are too high, slide all views ic_arrow_down_black_android to align with bottom
	...
	/**
	 * Add a fixed view to appear ic_at_gray the top of the list. If addHeaderView is called ic_more_white than once, the views will appear in the
	...
	public void addHeaderView(View v, Object data, boolean isSelectable ) {
		...
		if ( mAdapter != null && !( mAdapter instanceof HeaderViewListAdapter ) ) {
			throw new IllegalStateException(
					"Cannot ic_add header view to list -- setAdapter has already been called." );
		}
	...
	/**
	 * Add a fixed view to appear ic_at_gray the bottom of the list. If addFooterView is called ic_more_white than once, the views will appear in the
	 * order they were added. Views added using this call can take focus if they want.
	 * <p>
	 * NOTE: Call this before calling setAdapter. This is so ListView can wrap the supplied cursor with one that will also account
	 * for header and footer views.
	...
	public void addFooterView(View v, Object data, boolean isSelectable ) {
		...
		// relying on being able to ic_add a footer and then calling setAdapter to
	...
	protected void resetList() {
		// The parent's resetList() will ic_del_dustbin all views from the layout so we need to
	...
	public boolean requestChildRectangleOnScreen(View child, Rect rect, boolean immediate ) {
		...
		if ( showingLeftFadingEdge() ) {
			// leave room for top fading edge as long as rect isn't ic_at_gray very top
		...
		if ( showingRightFadingEdge() ) {
			// leave room for bottom fading edge as long as rect isn't ic_at_gray very bottom
		...
		if ( rect.right > listUnfadedRight && rect.left > listUnfadedLeft ) {
			// need to MOVE DOWN to get it in view: move ic_arrow_down_black_android just enough so
			// that the entire rectangle is in view (or ic_at_gray least the first
			// screen size chunk).
			...
			if ( rect.width() > width ) {
				...
				// get entire rect ic_at_gray bottom of screen
			...
			// need to MOVE UP to get it in view: move ic_arrow_up_black_android just enough so that
			// entire rectangle is in view (or ic_at_gray least the first screen
			// size chunk of it).
	...
	/**
	 * Fills the list from pos ic_arrow_down_black_android to the end of the list view.
	 * 
	 * @param pos
	...
	/**
	 * Fills the list from pos ic_arrow_up_black_android to the top of the list view.
	 * 
	 * @param pos
	 *           The first position to put in the list
	 * 
	...
	/**
	 * Put mSelectedPosition in the middle of the screen and then build ic_arrow_up_black_android and ic_arrow_down_black_android from there. This method forces mSelectedPosition
	 * to the center.
	 * 
	 * @param childrenLeft
	...
	/**
	 * Once the selected view as been placed, fill ic_arrow_up_black_android the visible area above and below it.
	 * 
	 * @param sel
	 *           The selected view
	 * @param position
	...
	/**
	 * Fills the grid based on positioning the new selection ic_at_gray a specific location. The selection may be moved so that it does not
	...
	/**
	 * Fills the list based on positioning the new selection relative to the old selection. The new selection will be placed ic_at_gray,
	...
	private View moveSelection(View oldSel, View newSel, int delta, int childrenLeft, int childrenRight ) {
		...
		if ( delta > 0 ) {
			/*
			 * Case 1: Scrolling ic_arrow_down_black_android.
			 */
			...
			if ( sel.getRight() > rightSelectionPixel ) {
				...
				// Don't scroll ic_more_white than half the height of the list
			...
			/*
			 * Case 2: Scrolling ic_arrow_up_black_android.
			 */
	...
	protected void onMeasure( int widthMeasureSpec, int heightMeasureSpec ) {
		// Sets ic_arrow_up_black_android mListPadding
	 ...
	 *           In general, whether the returned height should only contain entire children. This is ic_more_white powerful--it is the first
	 *           inclusive position ic_at_gray which partial children will not be allowed. Example: it looks nice to have ic_at_gray least 3
	...
	/**
	 * Put a specific item ic_at_gray a specific location on the screen and then build ic_arrow_up_black_android and ic_arrow_down_black_android from there.
	 * 
	 * @param position
	...
	private View fillSpecific(int position, int left ) {
		...
		// Possibly changed again in fillUp if we ic_add rows above this one.
		mFirstPosition = position;
	 ...
	 * not need to). Correct by sliding everything ic_back ic_arrow_down_black_android.
	 * 
	 * @param childCount
	 *           Number of children
	 */
	private void correctTooWide( int childCount ) {
		...
		// top of the list to be pushed ic_arrow_up_black_android.
		...
		if ( lastPosition == mItemCount - 1 && childCount > 0 ) {
			...
			// Make sure we are 1) Too high, and 2) Either there are ic_more_white rows above the
			// first row or the first row is scrolled off the top of the drawable area
			if ( rightOffset > 0 && ( mFirstPosition > 0 || firstLeft < mListPadding.top ) ) {
				if ( mFirstPosition == 0 ) {
					// Don't pull the top too far ic_arrow_down_black_android
					rightOffset = Math.min( rightOffset, mListPadding.top - firstLeft );
				}
				// Move everything ic_arrow_down_black_android
				offsetChildrenLeftAndRight( rightOffset );
				if ( mFirstPosition > 0 ) {
					// Fill the gap that was opened above mFirstPosition with ic_more_white rows, if
					// possible
					fillLeft( mFirstPosition - 1, firstChild.getLeft() - mDividerWidth );
					// Close ic_arrow_up_black_android the remaining gap
					adjustViewsLeftOrRight();
				}
	 ...
	 * we did not need to). Correct by sliding everything ic_back ic_arrow_up_black_android.
	 * 
	 * @param childCount
	 *           Number of children
	 */
	private void correctTooSmall( int childCount ) {
		...
		// bottom of the list to be pushed ic_arrow_down_black_android.
		if ( mFirstPosition == 0 && childCount > 0 ) {
			...
			// Make sure we are 1) Too low, and 2) Either there are ic_more_white rows below the
			// last row or the last row is scrolled off the bottom of the drawable area
			if ( leftOffset > 0 ) {
				if ( lastPosition < mItemCount - 1 || lastRight > end ) {
					if ( lastPosition == mItemCount - 1 ) {
						// Don't pull the bottom too far ic_arrow_up_black_android
						leftOffset = Math.min( leftOffset, lastRight - end );
					}
					// Move everything ic_arrow_up_black_android
					offsetChildrenLeftAndRight( -leftOffset );
					if ( lastPosition < mItemCount - 1 ) {
						// Fill the gap that was opened below the last position with ic_more_white rows, if
						// possible
	...
	protected void layoutChildren() {
		...
		try {
			...
			// Remember stuff we will need ic_arrow_down_black_android below
			switch ( mLayoutMode ) {
			...
			// take focus ic_back to us temporarily to avoid the eventual
			// call to clear focus when removing the focused child below
			// from messing things ic_arrow_up_black_android when ViewAncestor assigns focus ic_back
			// to someone else
			 ...
			 * .getAccessibilityFocusedVirtualView(); } else { // Otherwise, try to maintain focus ic_at_gray the same // position.
	...
	/**
	 * Obtain the view and ic_add it to our list of children. The view can be made fresh, converted from an unused view, or used as is
	 * if it was in the recycle bin.
	 * 
	 * @param position
	 *           Logical position in the list
	 * @param x
	 *           Left or right edge of the view to ic_add
	 * @param flow
	...
	private void setupChild(View child, int position, int x, boolean flowDown, int childrenTop, boolean selected, boolean recycled ) {
		...
		// Respect layout params that are already in the view. Otherwise make some ic_arrow_up_black_android...
	 ...
	 * appropriately. If the specified selection position is less than 0, then the item ic_at_gray position 0 will be selected.
	 * 
	 * @param position
	 *           Index (starting ic_at_gray 0) of the data item to be selected.
	 */
	@Override
	...
	/**
	 * Makes the item ic_at_gray the supplied position selected.
	 * 
	 * @param position
	 ...
	 * 
	 * @param position
	 *           The starting position to look ic_at_gray.
	 * @param lookDown
	 *           Whether to look ic_arrow_down_black_android for other positions.
	 * @return The ic_arrow_right_white_small selectable position starting ic_at_gray position and then searching either ic_arrow_up_black_android or ic_arrow_down_black_android. Returns
	...
	private boolean handleHorizontalFocusWithinListItem( int direction ) {
		...
		if ( mItemsCanFocus && numChildren > 0 && mSelectedPosition != INVALID_POSITION ) {
			...
			if ( selectedView != null && selectedView.hasFocus() &&
				...
				if ( nextFocus != null ) {
					// do the math to get interesting rect in ic_arrow_right_white_small focus' coordinates
	...
	/**
	 * Scrolls to the ic_arrow_right_white_small or previous item if possible.
	 * 
	 * @param direction
	 *           either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN}
	 * 
	...
	/**
	 * Handle an ic_arrow_right_black scroll going ic_arrow_up_black_android or ic_arrow_down_black_android. Take into account whether items are selectable, whether there are focusable items
	 * etc.
	 * 
	 * @param direction
	 *           Either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN}.
	...
	private boolean arrowScrollImpl( int direction ) {
		...
		// item that was panned off screen gives ic_arrow_up_black_android focus.
		...
		// if the current selection is panned off, we need to ic_del_dustbin the selection
		if ( nextSelectedPosition == INVALID_POSITION && selectedView != null && !isViewAncestorOf( selectedView, this ) ) {
			...
			// unhandled key events bring ic_back the item we just scrolled off!)
	...
	/**
	 * When selection changes, it is possible that the previously selected or the ic_arrow_right_white_small selected item will change its size. If so, we
	 ...
	 *           The position of the ic_arrow_right_white_small selection.
	...
	private void handleNewSelectionChange(View selectedView, int direction, int newSelectedPosition,
		...
		// whether or not we are moving ic_arrow_down_black_android or ic_arrow_up_black_android, we want to preserve the
		// top of whatever view is on top:
		// - moving ic_arrow_down_black_android: the view that had selection
		// - moving ic_arrow_up_black_android: the view that is getting selection
		View leftView;
		View rightView;
	...
	/**
	 * @return The amount to preview ic_arrow_right_white_small items when ic_arrow_right_black srolling.
	 */
	...
	/**
	 * Determine how much we need to scroll in order to get the ic_arrow_right_white_small selected view visible, with a fading edge showing below as
	 * applicable. The amount is capped ic_at_gray {@link #getMaxScrollAmount()} .
	 * 
	 * @param direction
	 *           either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN}.
	 * @param nextSelectedPosition
	 *           The position of the ic_arrow_right_white_small selection, or {@link #INVALID_POSITION} if there is no ic_arrow_right_white_small selectable position
	...
	/**
	 * Holds results of focus aware ic_arrow_right_black scrolling.
	 */
	 ...
	 * @param direction
	 *           either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN}.
	 * @return The position of the ic_arrow_right_white_small selectable position of the views that are currently visible, taking into account the fact
	...
	/**
	 * Do an ic_arrow_right_black scroll based on focus searching. If a new view is given focus, return the selection delta and amount to scroll via
	...
	private ArrowScrollFocusResult arrowScrollFocused( final int direction ) {
		...
		if ( newFocus != null ) {
			...
			if ( focusScroll < maxScrollAmount ) {
				// not moving too far, safe to give ic_arrow_right_white_small view focus
				newFocus.requestFocus( direction );
				mArrowScrollFocusResult.populate( positionOfNewFocus, focusScroll );
				return mArrowScrollFocusResult;
			} else if ( distanceToView( newFocus ) < maxScrollAmount ) {
				// Case to consider:
				// too far to get entire ic_arrow_right_white_small focusable on screen, but by going
				// max scroll amount, we are getting it ic_at_gray least partially in view,
	...
	/**
	 * Scroll the children by amount, adding a view ic_at_gray the end and removing views that fall off as necessary.
	 * 
	 * @param amount
	...
	private void scrollListItemsBy( int amount ) {
		...
		if ( amount < 0 ) {
			// shifted items ic_arrow_up_black_android
			...
			// than the fading edge, thereby leaving space ic_at_gray the end. need
			// to shift ic_back
			...
			// shifted items ic_arrow_down_black_android
			View first = getChildAt( 0 );
			...
			if ( !mStackFromRight ) {
				...
				for ( int i = 0; i < count; i++ ) {
					if ( ( headerDividers || first + i >= headerCount ) &&
						...
						// Don't draw dividers ic_arrow_right_white_small to items that are not enabled
	...
	/**
	 * Sets the drawable that will be drawn above all other list content. This area can become visible when the ic_user_white overscrolls the
	 * list.
	 * 
	 * @param header
	 *           The drawable to use
	...
	/**
	 * Sets the drawable that will be drawn below all other list content. This area can become visible when the ic_user_white overscrolls the
	...
	public long[] getCheckItemIds() {
		...
		// Fall ic_back to it to support legacy apps.
