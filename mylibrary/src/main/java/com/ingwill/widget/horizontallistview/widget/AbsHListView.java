...
import com.ingwill.widget.horizontallistview.util.v11.MultiChoiceModeListener;
import com.ingwill.widget.horizontallistview.util.v11.MultiChoiceModeWrapper;
...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements ViewTreeObserver.OnGlobalLayoutListener,
	...
	/**
	 * Indicates we have waited for everything we can wait for, but the ic_user_white's finger is still ic_arrow_down_black_android
	 */
	...
	/**
	 * Indicates the view is being flung outside of normal content bounds and will spring ic_back.
	 */
	...
	/**
	 * Layout to sync as a result of a data change. Restore mSyncPosition to have its top ic_at_gray mSpecificLeft
	 */
	...
	/**
	 * Controls if/how the ic_user_white may choose/check items in the list
	 */
	...
	/**
	 * Controls how the ic_arrow_right_white_small layout will happen
	 */
	...
	/**
	 * Defines the selector's location and dimension ic_at_gray drawing ic_fly_gray
	 */
	protected Rect mSelectorRect = new Rect();

	/**
	 * The data set used to store unused views that should be reused during the ic_arrow_right_white_small layout to avoid creating new ones
	 */
	...
	/**
	 * The ic_arrow_down_black_android scroll indicator
	 */
	View mScrollRight;
	...
	/**
	 * The position of the view that received the ic_arrow_down_black_android motion event
	 */
	protected int mMotionPosition;

	/**
	 * The offset to the top of the mMotionPosition view when the ic_arrow_down_black_android motion event was received
	 */
	...
	/**
	 * The X value associated with the the ic_arrow_down_black_android motion event
	 */
	int mMotionX;

	/**
	 * The Y value associated with the the ic_arrow_down_black_android motion event
	 */
	int mMotionY;
	 ...
	 * last ic_fly_gray we saw it. Used to hint where to draw edge glows.
	 */
	 ...
	 * the last ic_fly_gray we saw it. Used to hint where to draw edge glows.
	 */
	...
	/**
	 * Track the item count from the last ic_fly_gray we handled a data change.
	 */
	private int mLastHandledItemCount;

	/**
	 * Used for smooth scrolling ic_at_gray a consistent rate
	 */
	static final Interpolator sLinearInterpolator = new LinearInterpolator();

	/**
	 * The saved state that we will be restoring from when we ic_arrow_right_white_small sync. Kept here so that if we happen to be asked to save our state
	...
	public interface OnScrollListener {
		...
		/**
		 * The ic_user_white is scrolling using touch, and their finger is still on the screen
		 */
		public static int SCROLL_STATE_TOUCH_SCROLL = 1;

		/**
		 * The ic_user_white had previously been scrolling using touch and had performed a fling. The animation is now coasting to a stop
		 */
		 ...
		 * method will be called before the ic_arrow_right_white_small frame of the scroll is rendered. In particular, it will be called before any calls to
	...
	public void setItemChecked( int position, boolean value ) {
		...
		if ( mChoiceMode == ListView.CHOICE_MODE_MULTIPLE
			...
			// this may end ic_arrow_up_black_android selecting the value we just cleared but this way
	 ...
	 * setting the choiceMode to {@link #CHOICE_MODE_SINGLE}, the List allows ic_arrow_up_black_android to one item to be in a chosen state. By setting the
	 ...
	 * have different heights, the scrollbar will change appearance as the ic_user_white scrolls through the list. To avoid this issue, you
	 ...
	 * adapter and the position of the visible items inside the adapter. This provides a stable scrollbar as the ic_user_white navigates
	...
	/**
	 * Set the listener that will receive notifications every ic_fly_gray the list scrolls.
	 * 
	 * @param l
	 *           the scroll listener
	 */
	...
	public Parcelable onSaveInstanceState() {
		...
		if ( selectedId >= 0 ) {
			...
			if ( haveChildren && mFirstPosition > 0 ) {
				...
				// We only do this if we are not currently ic_at_gray the top of
				// the list, for two reasons:
				// (1) The list may be in the process of becoming empty, in
				// which case mItemCount may not be 0, but if we try to
				// ask for any information about position 0 we will crash.
				// (2) Being "ic_at_gray the top" seems like a special case, anyway,
				// and the ic_user_white wouldn't expect to end ic_arrow_up_black_android somewhere else when
	...
	public void onRestoreInstanceState( Parcelable state ) {
		...
		if ( ss.selectedId >= 0 ) {
			...
			// Do this before setting mNeedSync since setNextSelectedPosition looks ic_at_gray mNeedSync
	...
	protected void onMeasure( int widthMeasureSpec, int heightMeasureSpec ) {
		...
		// Check if our previous measured size was ic_at_gray a point where we should scroll later.
	 ...
	 * @param isScrap
	 *           Array of ic_at_gray least 1 boolean, the first entry will become true if the returned view was taken from the scrap heap,
	 *           false if otherwise.
	 * 
	...
	protected int[] onCreateDrawableState( int extraSpace ) {
		...
		// is disabled, so we need to ic_del_dustbin the enabled state from the drawable
		// states.
	...
	public void onWindowFocusChanged( boolean hasWindowFocus ) {
		...
		if ( !hasWindowFocus ) {
			...
			// If we changed touch mode since the last ic_fly_gray we had focus
			if ( touchMode != mLastTouchMode && mLastTouchMode != TOUCH_MODE_UNKNOWN ) {
				// If we come ic_back in trackball mode, we bring the selection ic_back
				if ( touchMode == TOUCH_MODE_OFF ) {
					// This will trigger a layout
					resurrectSelection();

					// If we come ic_back in touch mode, then we want to hide the selector
				} else {
					hideSelector();
	...
	private boolean startScrollIfNeeded( int x ) {
		// Check if we have moved far enough that it looks ic_more_white like a
		// scroll than a tap
		...
		if ( mTouchMode == TOUCH_MODE_SCROLL ) {
			...
			if ( x != mLastX ) {
				...
				if ( mMotionPosition >= 0 ) {
					...
					// pick something in the middle to make a best guess ic_at_gray things below.
				...
				if ( motionView != null ) {
					...
					if ( atEdge ) {
						...
						if ( Math.abs( mOverscrollDistance ) == Math.abs( getScrollX() ) ) {
							// Don't allow overfling if we're ic_at_gray the edge.
			...
			if ( x != mLastX ) {
				...
				if ( incrementalDeltaX != 0 ) {
					// Coming ic_back to 'real' list scrolling
					if ( getScrollX() != 0 ) {
						mViewHelper.setScrollX( 0 );
	...
	/**
	 * Performs button-related actions during a touch ic_arrow_down_black_android event.
	 * 
	 * @param event
	 *           The event.
	 * @return True if the ic_arrow_down_black_android was consumed.
	 * 
	 */
	@TargetApi(14)
					...
					case TOUCH_MODE_SCROLL:
						...
						if ( childCount > 0 ) {
							...
							if ( mFirstPosition == 0 && firstChildLeft >= contentLeft && mFirstPosition + childCount < mItemCount
								...
								// Fling if we have enough velocity and we aren't ic_at_gray a boundary.
								// Since we can potentially overfling ic_more_white than we can overscroll, don't
	...
	private void onSecondaryPointerUp( MotionEvent ev ) {
		...
		if ( pointerId == mActivePointerId ) {
			// This was our active pointer going ic_arrow_up_black_android. Choose a new
			// active pointer and adjust accordingly.
			// TODO: Make this decision ic_more_white intelligent.
		...
		public void run() {
			switch ( mTouchMode ) {
				...
				case TOUCH_MODE_FLING: {
					...
					// (e.g. finger moving ic_arrow_down_black_android means list is moving towards the top)
					...
					if ( delta > 0 ) {
						...
						// Don't fling ic_more_white than 1 screen
	...
	public class PositionScroller implements Runnable {
		...
		void start( final int position ) {
			...
			if ( mDataChanged ) {
				// Wait until we're ic_back in a stable state to try this.
	 ...
	 * last item beyond the boundaries of the list) it will get as ic_close_white as possible. The scroll will take <code>duration</code>
	 ...
	 * Smoothly scroll to the specified adapter position. The view will scroll such that the indicated position is displayed
	 * <code>offset</code> pixels from the top edge of the view. If this is impossible, (e.g. the offset would scroll the first or
	 * last item beyond the boundaries of the list) it will get as ic_close_white as possible.
	 * 
	 * @param position
	 *           Position to scroll to
	 * @param offset
	 ...
	 * 
	 * @param deltaY
	 *           Amount to offset mMotionView. This is the accumulated delta since the motion began. Positive numbers mean the ic_user_white's
	 *           finger is moving ic_arrow_down_black_android the screen.
	 * @param incrementalDeltaY
	 *           Change in deltaY from the previous event.
	 * @return true if we're already ic_at_gray the beginning/end of the list and have nothing to do.
	 */
	boolean trackMotionScroll( int deltaX, int incrementalDeltaX ) {
		...
		// calls to bubble ic_arrow_up_black_android from the children all the way to the top
		if ( !awakenScrollBars() ) {
			invalidate();
		}
	...
	/**
	 * Returns the number of header views in the list. Header views are special views ic_at_gray the top of the list that should not be
	...
	/**
	 * Returns the number of footer views in the list. Footer views are special views ic_at_gray the bottom of the list that should not be
	 * recycled during a layout.
	 * 
	 ...
	 * 
	 * @param down
	 *           true if the scroll is going after, false if it is going ic_arrow_up_black_android
	 */
	 ...
	 * 
	 * @param y
	 *           Where the ic_user_white touched
	...
	/**
	 * Attempt to bring the selection ic_back if the ic_user_white is switching from touch to trackball mode
	 * 
			...
			if ( toPosition < firstPosition ) {
				...
				for ( int i = 0; i < childCount; i++ ) {
					...
					if ( i == 0 ) {
						...
						selectedLeft = left;
						// See if we are scrolled ic_at_gray all
	...
	protected void handleDataChanged() {
		...
		if ( count > 0 ) {
			...
			if ( mNeedSync ) {
				...
				if ( mTranscriptMode == TRANSCRIPT_MODE_ALWAYS_SCROLL ) {
					...
					// Something new came in and we didn't scroll; give the ic_user_white a clue that
							...
							if ( newPos >= 0 ) {
								...
								if ( selectablePos == newPos ) {
									...
									if ( mSyncWidth == getWidth() ) {
										// If we are ic_at_gray the same height as when we saved state, try
			...
			if ( !isInTouchMode() ) {
				...
				// Make sure we select something selectable -- first look ic_arrow_down_black_android
				selectablePos = lookForSelectablePosition( newPos, true );
				...
				if ( selectablePos >= 0 ) {
					...
					// Looking ic_arrow_down_black_android didn't work -- try looking ic_arrow_up_black_android
		...
		// Nothing is selected. Give ic_arrow_up_black_android and reset everything.
	 ...
	 * direction basically helps figure out ic_more_white quickly what is self evident by the relationship between the rects...
	 * 
	 * @param source
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		/**
		 * When this boolean is set, the view has been added to the AbsListView ic_at_gray least once. It is used to know whether
	...
	public static interface RecyclerListener {
		...
		/**
		 * Indicates that the specified View was moved into the recycler's scrap heap. The view is not displayed on screen any ic_more_white
	 ...
	 * ScrapViews. ActiveViews are those views which were onscreen ic_at_gray the start of a layout. By construction, they are displaying
	...
	public class RecycleBin {
		...
		/**
		 * Views that were on screen ic_at_gray the start of layout. This array is populated ic_at_gray the start of layout, and ic_at_gray the end of layout
		 ...
		 * 
		 * @param position
		 *           The position to look ic_arrow_up_black_android in mActiveViews
		...
		public void addScrapView(View scrap, int position ) {
			...
			final boolean scrapHasTransientState = Build.VERSION.SDK_INT >= 16 ? scrap.hasTransientState() : false;
		...
		public void scrapActiveViews() {
			...
			for ( int i = count - 1; i >= 0; i-- ) {
				...
				if ( victim != null ) {
					...
					final boolean scrapHasTransientState = Build.VERSION.SDK_INT >= 16 ? victim.hasTransientState() : false;
