...
package com.ingwill.widget.clip;
...
public class ClipRectLayout extends RelativeLayout implements OnTouchListener{
    private static final  float MAX_SCALE = 10.0f;
    private static final int DEFAULT_SIZE = 400;

    private ClipRectView mClipView;
    ...
    public ClipRectLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ClipRectLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.clip_rect_picture_layout,
                this);
        mClipView = (ClipRectView) findViewById(R.id.clipview);
        mSouceImageView = (ImageView) findViewById(R.id.src_pic);
        mSouceImageView.setOnTouchListener(this);
        findViewById(R.id.bt_roate_left).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                rotate(-90.0f);
            }
        });
        findViewById(R.id.bt_roate_right).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                rotate(90.0f);
            }
        });
        mClipViewWidth =(int)(dm.widthPixels*0.99); //dm.widthPixels * 2 / 3;
        mClipViewHeight =(int)(dm.widthPixels*0.99); //dm.widthPixels * 2 / 3;
    ...
    public void setSourceImage(Bitmap bitmap, Window window) {
        ...
        float scaleWidth =1;  //(float) (mClipViewWidth) / (float) srcWidth;
        float scaleHeight =1; //(float) (mClipViewHeight) / (float) srcHeight;
