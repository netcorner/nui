...
import com.ingwill.widget.listview.BaseLoadMoreFooter;
import com.ingwill.widget.listview.ListViewEx;
...
public class StickyListHeadersListView extends FrameLayout {
    ...
    public interface OnStickyHeaderOffsetChangedListener {
         ...
         *               therefor you should ic_user_white getMeasured* methods instead of
    ...
    // used to not have to call getHeaderId() all the ic_fly_gray
    ...
    public void setOnScrollListener(View mHeader, int mMinHeaderTranslation, View mFooter, int mMinFooterTranslation,BaseLoadMoreFooter baseLoadMoreFooter){
        mList.setOnScrollListener(new ReturnListViewOnScrollListener(mHeader,mMinHeaderTranslation,mFooter,mMinFooterTranslation,baseLoadMoreFooter));
    }



    public void setOnScrollListener(final BaseLoadMoreFooter baseLoadMoreFooter){
        addFooterView(baseLoadMoreFooter.footerView, null, false);
        mList.setOnScrollListener(new WrapperListScrollListener(){
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                super.onScrollStateChanged(view,scrollState);
                if (view.getLastVisiblePosition() == view.getCount() - 1) {
                    baseLoadMoreFooter.checkLoadMore();
                }
            }
    ...
    // Reset values tied the header. also ic_del_dustbin header form layout
    ...
    private void updateOrClearHeader(int firstVisiblePosition) {
        ...
        // Most of the ic_fly_gray getFixedFirstVisibleItem() should be called
    ...
    private void updateHeader(int headerPosition) {
        ...
        // Skip looking ic_at_gray the first view. it never matters because it always
    ...
    private class ReturnListViewOnScrollListener extends WrapperListScrollListener {
        ...
        // region Constructors
        private ReturnListViewOnScrollListener(View mHeader, int mMinHeaderTranslation, View mFooter, int mMinFooterTranslation,BaseLoadMoreFooter baseLoadMoreFooter) {
        ...
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            super.onScrollStateChanged(view,scrollState);
            for (OnScrollListener listener : mExtraOnScrollListenerList) {
            ...
            if (scrollState == SCROLL_STATE_IDLE ) {
                ...
                if (-mHeaderDiffTotal > 0 && -mHeaderDiffTotal < midHeader) {
                    ...
                    anim.addListener(new Animator.AnimatorListener() {
                        ...
                        public void onAnimationEnd(Animator animator) {
                            if(mHeaderDiffTotal==0){
                                setStickyHeaderTopOffsetNew(mHeader.getHeight());
                            }else{
                                setStickyHeaderTopOffset(0);
                            }
                        }
                    ...
                    if(mPrevScrollY>-mMinHeaderTranslation) {
                ...
                if (-mFooterDiffTotal > 0 && -mFooterDiffTotal < midFooter) { // slide ic_arrow_up_black_android
                    ObjectAnimator anim = ObjectAnimator.ofFloat(mFooter, "translationY", mFooter.getTranslationY(), 0);
                    anim.setDuration(100);
                    anim.start();
                    mFooterDiffTotal = 0;
                } else if (-mFooterDiffTotal < mMinFooterTranslation && -mFooterDiffTotal >= midFooter) { // slide ic_arrow_down_black_android
        ...
        public void onScroll(AbsListView listview, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            if (diff != 0) {
                if (diff < 0) { // scrolling ic_arrow_down_black_android
                ...
                } else { // scrolling ic_arrow_up_black_android
                ...
                if(mHeaderDiffTotal==0){
                    setStickyHeaderTopOffset(mHeader.getHeight());
                }else{
                    setStickyHeaderTopOffset(0);
                }
        ...
        public void registerExtraOnScrollListener(OnScrollListener listener) {
    ...
    private class WrapperListScrollListener implements OnScrollListener {
        ...
        private int mHeaderDiffTotal = 0;
        private int mMinHeaderTranslation=200;
    ...
    private class WrapperViewListLifeCycleListener implements WrapperViewList.LifeCycleListener {
        ...
        public void onDispatchDrawOccurred(Canvas canvas) {
            // onScroll is not called often ic_at_gray all before froyo
    ...
    public void setStickyHeaderTopOffsetNew(int stickyHeaderTopOffset) {
        mStickyHeaderTopOffset = stickyHeaderTopOffset;
        int positioin=mList.getFixedFirstVisibleItem()-1;
        updateOrClearHeader(positioin);
    }
    ...
    public WrapperViewList getFullWrappedList() {
        return mList;
    }

    private boolean requireSdkVersion(int versionCode) {
        if (Build.VERSION.SDK_INT < versionCode) {
            Log.e("StickyListHeaders", "Api lvl must be ic_at_gray least " + versionCode + " to call this method");
            return false;
        }
        return true;
    }
    ...
    /**
     * @return true if the fast scroller will always show. False on ic_arrow_right_white-Honeycomb devices.
    ...
    public void setHeadScaleImage(View headView, FrameLayout layout_header_container, View mack_cover_imageView, ImageView cover_imageView, String bitmap, int dp){
        mList.addHeaderImageView(headView,layout_header_container,mack_cover_imageView,cover_imageView,dp);


        Glide.with(getContext()).asBitmap().load(bitmap).centerCrop().placeholder(R.drawable.reppic).into(new SimpleTarget<Bitmap>() {
            @Override
            public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                if(resource.getHeight()>resource.getWidth()){
                    mList.setCover_imageViewheightMaxHeight((int)(resource.getHeight()/resource.getWidth()*mList.MAXHEIGHTSCALE));
                }
                mList.getCover_imageView().setImageBitmap(resource);
            }
        }); //方法中设置asBitmap可以设置回调类型



//        Glide.with(getContext()).load(bitmap).asBitmap().centerCrop().placeholder(R.drawable.ic_reppic).into(new SimpleTarget<Bitmap>() {
//            @Override
...
//        }); //方法中设置asBitmap可以设置回调类型
        ...
        //mList.setHeadScaleImage(bitmap);
    }
