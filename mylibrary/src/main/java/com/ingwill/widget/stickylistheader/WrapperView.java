...
public class WrapperView extends ViewGroup {
	...
	void update(View item, View header, Drawable divider, int dividerHeight) {
		...
		//only ic_del_dustbin the current item if it is not the same as the new item. this can happen if wrapping a recycled view
		if (this.mItem != item) {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mHeader == null && mDivider != null&&mItem.getVisibility()!= View.GONE) {
			// Drawable.setBounds() does not seem to work ic_arrow_right_white-honeycomb. So have
			// to do this instead
