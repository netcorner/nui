...
package com.ingwill.widget.stickygridheaders;
...
import com.ingwill.widget.stickylistheader.StickyListHeadersListView;
 ...
 */
public class StickyGridHeadersGridView extends GridViewWithHeaderAndFooter implements OnScrollListener,
    ...
    /**
     * Gets the header ic_at_gray an item position. However, the position must be that
     * of a HeaderFiller.
     * 
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (adapter instanceof StickyGridHeadersBaseAdapter) {
            ...
            // Wrap ic_arrow_up_black_android simple adapter to auto-generate the data we need.
            baseAdapter = new StickyGridHeadersSimpleAdapterWrapper(
                    (StickyGridHeadersSimpleAdapter)adapter);
        } else {
            // Wrap ic_arrow_up_black_android a list adapter so it is an adapter with zero headers.
    ...
    private void scrollChanged(int firstVisibleItem) {
        ...
        if (childCount != 0) {
            ...
            // Find the ic_arrow_right_white_small header after the stickied one.
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (int i = 0; i < headerPositions.size(); i++) {
            ...
            if(frame instanceof StickyGridHeadersBaseAdapterWrapper.HeaderFillerView) {
                View header;
                try {
