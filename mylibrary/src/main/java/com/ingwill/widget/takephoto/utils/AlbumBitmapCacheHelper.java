package com.ingwill.widget.takephoto.utils;
...
//        cr = AppContext.getInstance().getContentResolver();
     ...
     */
    public Bitmap getBitmap(Context context, final String path, int width, int height, final ILoadImageCallback callback, Object... objects){
        Bitmap bitmap = getBitmapFromCache(path, width, height);
        //如果能够从缓存中获取符合要求的图片，则直接回调
        if (bitmap != null) {
            //Log.e("zhao", "get bitmap from cache");
        } else {
            decodeBitmapFromPath(context,path, width, height, callback, objects);
        }
        return bitmap;
    }
...
//    ExecutorService tpe = Executors.newFixedThreadPool(1);
     ...
     * 通过path获取图片bitmap
     */
    private void decodeBitmapFromPath(final Context context, final String path, final int width, final int height, final ILoadImageCallback callback, final Object... objects) throws OutOfMemoryError {
        ...
        tpe.execute(new Runnable() {
            ...
            public void run() {
                ...
                int degree=ImageTools.getBitmapDegree(path);
                Bitmap bitmap = null;
                //返回大图,屏幕宽度为准
                if (width == 0 || height == 0) {
                    ...
                    options.inSampleSize = computeScale(options, ((WindowManager) (context.getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth(), ((WindowManager) (context.getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth());
                    ...
                    if(degree!=0){
                        bitmap=ImageTools.rotateImage(bitmap,degree);
                    }
                } else {
                    //返回小图，第一步，从temp目录下取该图片指定大小的缓存，如果取不到，
                    // 第二步，计算samplesize,如果samplesize > 4,
                    // 第三步则将压缩后的图片存入temp目录下，以便下次快速取出
                    String hash = StringUtils.md5(path+"_"+width+"_"+height);
                    File file = new File(AndroidUtils.getDataPath(context));
                    if (!file.exists())
                        file.mkdirs();
                    //临时文件的文件名
                    String tempPath = AndroidUtils.getDataPath(context) + hash + ".temp";
