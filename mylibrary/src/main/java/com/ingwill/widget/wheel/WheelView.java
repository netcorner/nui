package com.ingwill.widget.wheel;
...
import com.ingwill.widget.wheel.adapter.ArrayWheelAdapter;
import com.ingwill.widget.wheel.adapter.WheelAdapter;
import com.ingwill.widget.wheel.interfaces.IPickerViewData;
import com.ingwill.widget.wheel.listener.LoopViewGestureListener;
import com.ingwill.widget.wheel.listener.OnItemSelectedListener;
import com.ingwill.widget.wheel.timer.InertiaTimerTask;
import com.ingwill.widget.wheel.timer.MessageHandler;
import com.ingwill.widget.wheel.timer.SmoothScrollTimerTask;
...
public class WheelView extends View {
    ...
    public final void setCurrentItemByPrimary(Object primaryValue) {
        if(adapter instanceof ArrayWheelAdapter){
            Map<Object,Integer> dir = ((ArrayWheelAdapter)adapter).getDir();
            if(dir.containsKey(primaryValue)){
                int index=dir.get(primaryValue);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (dividerType == DividerType.WRAP) {//横线长度仅包裹内容
            ...
            int px=AndroidUtils.dip2px(context,2);
            for(int i=0;i<px;i++) {
                canvas.drawLine(startX, firstLineY+i, endX, firstLineY+i, paintIndicator);
                canvas.drawLine(startX, secondLineY+i, endX, secondLineY+i, paintIndicator);
            }
        } else {
            int px=AndroidUtils.dip2px(context,2);
            for(int i=0;i<px;i++) {
                canvas.drawLine(0.0F, firstLineY+i, measuredWidth, firstLineY+i, paintIndicator);
                canvas.drawLine(0.0F, secondLineY+i, measuredWidth, secondLineY+i, paintIndicator);
            }
        }
